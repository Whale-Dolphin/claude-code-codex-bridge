...
| --- | --- | --- | --- | --- | --- | --- |
| CC client | Default and five efforts preserve model/level | Loopback SSE responder with dummy auth | Fable/Astra plus Opus/Sol, Sonnet/Terra, and Haiku/Luna; adaptive thinking, exact effort; default xhigh | `python3 scripts/verify_effort.py client` | CC/wrapper changes | Implemented; run on target host |
...
| Real upstream | Five efforts and requested tier survive bridge/upstream | Twenty short Responses requests | Canonical model, matching returned effort, exact response, and returned tier matches the requested tier | `python3 scripts/verify_effort.py upstream` for a client profile, or add `--base-url` and `--key-file` | Bridge/config changes | Implemented; consumes account usage |
...
The upstream stage calls the selected bridge's Responses endpoint directly. With no overrides it reads `~/.config/claudex/proxy-url` and `proxy.key`. A `proxy-only` host without a client profile must pass `--base-url` plus `--key-file` or `--key-stdin`. It checks returned `reasoning.effort`, canonical model, exact output, and the completed response's actual `service_tier`, not the model's self-description. The verifier requests `service_tier: priority` for Sol, Terra, and Luna and `default` for Astra. A GPT-5.6 response with `service_tier: default`, `auto`, or no tier metadata fails the stage. This stage alone does not prove that a proxy-wide GPT-5.6 override is installed, that CC emitted the right effort, or that the Anthropic translation path works; inspect the proxy configuration and run the client and full E2E stages when a client is installed.

Do not treat catalog capability as runtime proof. CLIProxyAPI can advertise a non-empty `service_tiers` list while its HTTP/SSE downstream still reaches Codex over an upstream HTTP transport that serves the standard tier. The [CLIProxyAPI HTTP/SSE Priority transport report](https://github.com/router-for-me/CLIProxyAPI/issues/4586) documents this failure mode and the upstream WebSocket boundary. Require a completed response that reports `priority`, or an equally direct provider-side accounting signal, before declaring Fast active.
