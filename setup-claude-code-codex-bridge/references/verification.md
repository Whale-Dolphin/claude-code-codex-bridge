...
The upstream stage calls the selected bridge's Responses endpoint directly. With no overrides it reads `~/.config/claudex/proxy-url` and `proxy.key`. A `proxy-only` host without a client profile must pass `--base-url` plus `--key-file` or `--key-stdin`. It checks returned `reasoning.effort` and canonical model, not the model's self-description. An upstream response with `service_tier: default` means Priority was not confirmed even when the Fast alias requested it. This stage alone does not prove CC emitted the right effort or that the Anthropic translation path works; run the client and full E2E stages too when a client is installed.
