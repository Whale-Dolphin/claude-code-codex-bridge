...
| --- | --- | --- | --- | --- | --- | --- |
| CC client | Default and five efforts preserve model/level | Loopback SSE responder with dummy auth | Fable/Astra and Opus/canonical Sol, adaptive thinking, exact effort; default xhigh | `python3 scripts/verify_effort.py client` | CC/wrapper changes | Implemented; run on target host |
...
| Real upstream | Five efforts and requested tier survive bridge/upstream | Ten short Responses requests | Canonical model, matching returned effort, exact response; Sol asks for Priority | `python3 scripts/verify_effort.py upstream` for a client profile, or add `--base-url` and `--key-file` | Bridge/config changes | Implemented; consumes account usage |
...
The upstream stage calls the selected bridge's Responses endpoint directly. With no overrides it reads `~/.config/claudex/proxy-url` and `proxy.key`. A `proxy-only` host without a client profile must pass `--base-url` plus `--key-file` or `--key-stdin`. It checks returned `reasoning.effort` and canonical model, not the model's self-description. The verifier explicitly requests `service_tier: priority` for Sol and `default` for Astra. A Sol response with `service_tier: default` or no tier metadata means Priority was not confirmed. This stage alone does not prove that a proxy-wide Sol override is installed, that CC emitted the right effort, or that the Anthropic translation path works; inspect the proxy configuration and run the client and full E2E stages when a client is installed.
